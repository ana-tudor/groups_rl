...
def perform_actions(observations, actions, rewards):

    # ob = env.reset()
    obs, acs, next_obs, terminals, image_obs = [], [], [], [], [], []
    steps = 0
    done_steps = 3
    for i in range(len(actions)):
        obs.append(observations)
        acs.append(actions[i])
        observations[:,-1] = acs
