...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, next_ac_na, reward_n, terminal_n):
        """ Implement DQN Loss """


        #Calculate a q value per action, then choose the predicted q value
        # as the one given the action taken

        q_t_values = self.q_net(ob_no) # Change to calculate 1 q value per state
        ...
        #Do not run loss backward yet

        return loss, qa_t_values, q_t_values


    def update(self, ob_no, ac_na, next_ob_no, next_ac_na, reward_n, terminal_n):
        """
        ...
        next_ac_na = ptu.from_numpy(next_ac_na).to(torch.long)
        ...
        # print(qa_t_values.size)
        # print(q_t_values.size)
        # q_t_logsumexp = torch.log(torch.sum(torch.exp(qa_t_values), axis=1))     # Might need to fix axis/dimension
        # cql_loss = torch.mean(q_t_logsumexp - q_t_values)    # Might need to fix q_t_values
